...
/* ==========================
   ...
   function spa_event_columns($columns) {
       return array(
           ...
           'event_dates'       => '📅 Dátum',
           'event_places'      => '📍 Miesta',
           'affects_training'  => '⚠️ Tréningy',
           'date'              => 'Vytvorené'
       );
   }
   
   add_action('manage_spa_event_posts_custom_column', 'spa_event_column_data', 10, 2);
   function spa_event_column_data($column, $post_id) {
       ...
       switch ($column) {
           ...
           case 'event_dates':
               ...
               if ($date_from && $date_to && $date_from !== $date_to) {
                   echo '<strong>' . esc_html($date_from) . '</strong><br>→ <strong>' . esc_html($date_to) . '</strong>';
               } elseif ($date_from) {
           ...
           case 'affects_training':
               ...
               if ($affects === 'yes') {
                   echo '<span style="background:#dc3232;color:#fff;padding:3px 8px;border-radius:3px;font-size:11px;font-weight:600;">✖ Nekonajú sa</span>';
               } else {
                   echo '<span style="color:#999;">—</span>';
               }
               break;
